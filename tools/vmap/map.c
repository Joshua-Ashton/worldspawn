...
#define FTE_CONT_LADDER         0x00004000
#define FTE_SURF_MESHCOLLISION  0x80000000
...
static void ParseRawBrush( qboolean onlyLights ){
	...
	while ( 1 )
	{
		...
		if ( onlyLights ) {
			...
			if (!strcmp(shader, "textures/CLIP")) {
				si->compileFlags |= C_DETAIL | C_TRANSLUCENT | C_NODRAW;
				si->contentFlags |= Q_CONT_PLAYERCLIP;
				si->surfaceFlags |= Q_SURF_NODRAW;
				si->contentFlags &= ~Q_CONT_SOLID;
				si->compileFlags &= ~C_SOLID;
			}

			if (!strcmp(shader, "textures/HINT")) {
				si->compileFlags |= C_HINT | C_NODRAW | C_TRANSLUCENT;
				si->surfaceFlags |= Q_SURF_NODRAW | Q_SURF_NONSOLID | Q_SURF_HINT;
				si->contentFlags &= ~Q_CONT_SOLID;
				si->compileFlags &= ~C_SOLID;
			}

			if (!strcmp(shader, "textures/AAATRIGGER")) {
				si->compileFlags |= C_DETAIL | C_TRANSLUCENT | C_NODRAW;
				si->contentFlags |= Q_CONT_TRIGGER;
				si->surfaceFlags |= Q_SURF_NODRAW;
				si->contentFlags &= ~Q_CONT_SOLID;
				si->compileFlags &= ~C_SOLID;
			}

			if (!strcmp(shader, "textures/SKY")) {
				si->surfaceFlags |= Q_SURF_SKY;
				si->compileFlags |= C_SKY;
			}

			if (!strcmp(shader, "textures/SKIP")) {
				si->compileFlags |= C_SKIP | C_NODRAW | C_TRANSLUCENT;
				si->surfaceFlags |= Q_SURF_NODRAW | Q_SURF_NONSOLID;
				si->contentFlags &= ~Q_CONT_SOLID;
				si->compileFlags &= ~C_SOLID;
			}
		}
